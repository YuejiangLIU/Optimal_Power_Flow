function [cvx_status,sol,exact_max] = socp_solver_CT(net)
%% parameter 
Nbus = net.Nbus;
Nline = net.Nline;
...
cvx_begin quiet
    ...
    %% objective 
    if net.ncost == 3
        minimize sum(sum(net.c_gen .* [(real(s_gen) * net.baseMVA).^2 (real(s_gen) * net.baseMVA) ones(size(real(s_gen)))]))
    elseif net.ncost == 2
        minimize sum(sum(net.c_gen .* [(real(s_gen) * net.baseMVA) ones(size(real(s_gen)))]))
    end
