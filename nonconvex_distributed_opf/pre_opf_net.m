...
% :: prepare 'net' data from raw network data 
...
% battery 
net.bat = zeros(net.Nbus,1);
net.p_bat_max = zeros(net.Nbus,1);      
net.p_bat_min = zeros(net.Nbus,1);
net.e_bat_max = zeros(net.Nbus,1);
net.e_bat_min = zeros(net.Nbus,1);
net.e_bat_midnight = zeros(net.Nbus,1);
for ii = 1:size(mpc.bat,1)
    idxBat = mpc.bat(ii,1);
    net.bat(idxBat) = 1;                % bus with generator 
    net.p_bat_max(idxBat) = mpc.bat(ii,2)/mpc.baseMVA;
    net.p_bat_min(idxBat) = mpc.bat(ii,3)/mpc.baseMVA;
    net.e_bat_max(idxBat) = mpc.bat(ii,4)/mpc.baseMVA;
    net.e_bat_min(idxBat) = mpc.bat(ii,5)/mpc.baseMVA;
    net.e_bat_midnight(idxBat) = mpc.bat(ii,6)/mpc.baseMVA;
end
