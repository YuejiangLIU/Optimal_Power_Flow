% opf case 7 centralized <- opf case 7 ori
...
%% Homotopy 
%Nmilestone = 5;             % number of sub tracking 
Nsubiter = Niter/Nmilestone;
niter = 0;
for nmilestone = 1:Nmilestone

milepercent = nmilestone/Nmilestone;  
demandnow = milepercent*demandnew + (1 - milepercent)*demandold;    
    
% Rated Load
p_load_rated = - demandnow*mpc.pf;
q_load_rated = - demandnow*sqrt(1-mpc.pf^2);

Nsubcurrentiter = 0;
for nsubiter = Nsubcurrentiter + 1:Nsubcurrentiter + Nsubiter 
%% ADMM x-update 
niter = niter + 1;
for ii = 1:Nbus
...
for ii = 1:Nbus
    ...
    res.B(niter,1) = res.B(niter,1) + sum( sum ( ( x(ii).B(1:4,1,:)-z(:,ii,:) ) .* ( x(ii).B(1:4,1,:)-z(:,ii,:) )) );
end
%res.B(niter) = sqrt(res.B(niter))/4;
...
for ii = 1:Nbus
    ...
    if Na > 0
        indA = mpc.A(ii,2);
        res.A(niter,1) = res.A(niter,1) + sum ( ( x(ii).A - z(1,indA,:) ).*( x(ii).A - z(1,indA,:) ) ); 
    end
    ...
    if Nc > 0
        for cc = 1:Nc
            indC = mpc.C(ii,cc+1);
            res.C(niter,1) = res.C(niter,1) + sum( sum ( ( x(ii).C(:,cc,:) - z(2:4,indC,:) ).*( x(ii).C(:,cc,:) - z(2:4,indC,:) ) ) );
        end
    end
end

%res.A(niter) = sqrt(res.A(niter));  % only one variable 
%res.C(niter) = sqrt(res.C(niter))/3;  % three variables 
resPrimal = sqrt(res.A(niter) + res.B(niter) + res.C(niter));
res.prim(niter,1) = resPrimal;
resDual = rho*sqrt(sum(sum(sum((z-zold).*(z-zold)))));
...
%close(h);
close all;
tHomotopy = toc(tStart);
loopTime = tHomotopy/Niter;
...
figPredName = strcat('consensus_predication_time_',num2str(t0),'_nsub_',num2str(Nmilestone),'.png');
...
figstr = strcat('consensus_predication_convergence_time_',num2str(t0),'_rho_',num2str(rho),'_nsub_',num2str(Nmilestone));
...
fprintf('Optimal value (Consensus Honotopy): %f \n', objTotal(end));
fprintf('Exactness (Consensus Honotopy): [%f %f]\n', x_check);
...
solfile = strcat('homo_sol_consensus_predication_time_',num2str(t0),'_rho_',num2str(rho),'_nsub_',num2str(Nmilestone),'.mat');
